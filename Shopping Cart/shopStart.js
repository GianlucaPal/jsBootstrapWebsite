...
function sub_selection(x) {
    if (quantities[x] <= 0) {} else { //checking if it is <= 0 then do nothing else subtract
        console.log(x);
        quantities[x] = quantities[x] - 1;
        totals[x] = prices[x] * quantities[x];
        totalOrderAmt -= prices[x];
...
document.getElementById("check").addEventListener("click", displayTotal);
...
function display_all() {
    ...
    for (i = 0; i < items.length; i++) {
        ...
        myTable += "<td><button onclick='sub_selection(" + i + ")'>Subtract</button></td>";
    }
